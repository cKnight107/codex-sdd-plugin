...
在执行本 skill 时，同时遵守 [../../references/full-sdd-lifecycle.md](../../references/full-sdd-lifecycle.md) 中 `spec-init` 阶段的协同规则。

- 用本插件 `skills/documentation-and-adrs/SKILL.md` 的心智约束目录职责与长期文档边界
- 用本插件 `skills/context-engineering/SKILL.md` 控制初始化阶段只读取最少上下文
- 本 skill 只负责搭建文档骨架，不替代后续 `spec-propose`、`spec-apply` 等阶段动作
...
7. 如果仓库已经存在与 `docs/` 等价的规范体系，先说明差异，再决定是复用还是新建版本化目录。
